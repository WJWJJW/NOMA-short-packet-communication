% Enhanced HRGP min d
function [sum_opt_M, opt_M, Hungarian_pairing]=En_HRGP_min_d(user_distance, N, K, target_BLER, rho, eta, lamda)
    user_idx = 1:2*K;
    UPG_idx = zeros(K,2);
    ...
    if rem(numel(re_pairing_idx_check(re_pairing_idx_check==1)), 2) == 1 % |RG|/2 is odd
        ...
        [~, min_d_min_idx] = min(re_pairing_idx_check .* d_min);
        re_pairing_idx_check(min_d_min_idx) = 0;
