...
w_UPG_opt_M_j = zeros(NNN,length(Pt));
middle_UPG_opt_M_j = zeros(NNN,length(Pt));
...
parfor u=1:length(ncluster)
    for jj=1:NNN
        ...
        [sum_En_UPG_opt_M_j(jj,u), B, ~] =...
            En_UPG_opt_delta(user_distance, NN, K, target_BLER, rho, eta, lamda);
        w_UPG_opt_M_j(jj,u) = max(B);
        middle_UPG_opt_M_j(jj,u) = B(ceil(K/2));
...
middle_UPG_opt_M = mean(middle_UPG_opt_M_j);
w_UPG_opt_M = mean(w_UPG_opt_M_j);
...
xlabel('Number of Cluster');
ylabel('Blocklength (Channel uses)');
legend('HAP','UPG', 'UPG worst cluster', 'OMA');
