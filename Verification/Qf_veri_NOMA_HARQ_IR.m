...
%     MAT_Epsilon1(u) = MAT_Epsilon12(u) + MAT_Epsilon11(u);
    ...
    Epsilon2(u) = 1 - ((alpha2*Xi_2*sqrt(M)*exp(1/(lambda2*rho*alpha1)))/(lambda2*rho*(alpha1^2)))* first_term_22...
    - ((Xi_2*sqrt(M))/alpha1)*second_term_22;
    
    Epsilon12(u) = 1 - ((alpha2*Xi_2*sqrt(M)*exp(1/(lambda1*rho*alpha1)))/(lambda1*rho*(alpha1^2)))* first_term_12...
    - ((Xi_2*sqrt(M))/alpha1)*second_term_12;
    
    
    Epsilon11(u) = 1 + Xi_1*sqrt(M)*lambda1*alpha1*rho*(exp(-tau_1/(lambda1*alpha1*rho))-exp(-nu_1/(lambda1*alpha1*rho)));
