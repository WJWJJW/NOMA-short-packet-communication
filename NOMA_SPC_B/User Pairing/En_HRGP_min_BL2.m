...
% Even odd pair
function [sum_opt_M, opt_M, Hungarian_pairing]=En_HRGP_min_BL2(user_distance, N, K, target_BLER, rho, eta, lamda)
    user_idx = 1:2*K;
    UPG_idx = zeros(K,2);
    re_pairing_idx_check = zeros(K,1);
    d_min = zeros(K,1);
    pre_blocklength = zeros(K,1);
    % Paring processing
    for ii = 1:K
        UPG_idx(ii,1) = user_idx(ii);
        UPG_idx(ii,2) = user_idx(K + ii);
        [pre_blocklength(ii)] = M_cal_Mod(N,[user_distance(ii),user_distance(K+ii)],1,...
                            [target_BLER(ii),target_BLER(K+ii)],rho,eta,lamda);
    ...
    if rem(numel(re_pairing_idx_check(re_pairing_idx_check==1)), 2) == 1 % |RG|/2 is odd
        re_pairing_idx_check(re_pairing_idx_check == 0) = NaN;
        [~, min_BL_idx] = min(re_pairing_idx_check .* pre_blocklength);
        re_pairing_idx_check(min_BL_idx) = 0;
        re_pairing_idx_check(isnan(re_pairing_idx_check)) = 0;
