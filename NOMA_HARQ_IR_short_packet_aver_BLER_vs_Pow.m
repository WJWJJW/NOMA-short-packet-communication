...
nu_1 = w1 - (1/(2*sqrt(M1)*Xi_1));
tau_1 = w1 + (1/(2*sqrt(M1)*Xi_1));
nu_2 = w2 - (1/(2*sqrt(M2)*Xi_2));
...
% 
%     MAT_Epsilon2(u) = Xi_2*sqrt(M2)*integral(fun1,nu_2,tau_2);
%     MAT_Epsilon12(u) = Xi_2*sqrt(M2)*integral(fun2,nu_2,tau_2);
%     MAT_Epsilon11(u) = Xi_1*sqrt(M1)*integral(fun3,nu_1,tau_1);
%     MAT_Epsilon1(u) = MAT_Epsilon12(u) + MAT_Epsilon11(u);
    ...
    Epsilon2(u) = 1 - ((alpha2*Xi_2*sqrt(M2)*exp(1/(lambda2*rho*alpha1)))/(lambda2*rho*(alpha1^2)))* first_term_22...
    - ((Xi_2*sqrt(M2))/alpha1)*second_term_22;
    
    Epsilon12(u) = 1 - ((alpha2*Xi_2*sqrt(M2)*exp(1/(lambda1*rho*alpha1)))/(lambda1*rho*(alpha1^2)))* first_term_12...
    - ((Xi_2*sqrt(M2))/alpha1)*second_term_12;
    
    
    Epsilon11(u) = 1 + Xi_1*sqrt(M1)*lambda1*alpha1*rho*(exp(-tau_1/(lambda1*alpha1*rho))-exp(-nu_1/(lambda1*alpha1*rho)));
...
semilogy(Pt, Epsilon2,'or', 'linewidth',1.5);
semilogy(Pt, Epsilon1, 'ob','linewidth',1.5);
semilogy(Pt, Epsilon12,'og', 'linewidth',1.5);
semilogy(Pt, Epsilon11, 'ok','linewidth',1.5);
...
figure (2)
% Plot order: Q-fun, linearization, high SNR approx
% Far user
semilogy(Pt, mean(q2,2),'r', 'linewidth', 1.5);
hold on; grid on;
...
legend('Far user simulation', 'Far user analytical Eq.(19)', 'Far user analytical Eq.(23)',...
       'Near user simulation', 'Near user analytical Eq.(20)', 'Near user analytical Eq.(24)');
